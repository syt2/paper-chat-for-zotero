/**
 * SessionSearchService - chat history search over SQLite session storage.
 *
 * Owns the two halves of history search that previously lived inside
 * SessionStorageService:
 *
 * 1. Foreground search: snapshot-stable queries over the indexed partition
 *    plus a source-projection fallback for rows the backfill has not reached.
 * 2. Background backfill: the invisible, adaptively-batched slice loop that
 *    projects `messages`/`session_meta` rows up to CURRENT_SEARCH_VERSION.
 *
 * The host (SessionStorageService) supplies initialization and transaction
 * execution; everything else is self-contained.
 */
...
export function projectMessageSearchTextSafe(
  ...
  try {
    ...
    ztoolkit.log(
      "[SessionSearchService] Message search projection failed, persisting without index:",
      message.id,
      error,
    );
...
export function projectTitleSearchTextSafe(
  ...
  try {
    ...
    ztoolkit.log(
      "[SessionSearchService] Title search projection failed, persisting without index:",
      error,
    );
    return { searchText: "", searchIndexVersion: 0, failed: true };
  }
...
/** Storage-side operations the search service borrows from its host. */
export interface SessionSearchHost {
  /** Ensure storage (including search state) is initialized. */
  init(): Promise<void>;
  /** Run an operation inside an exclusive storage transaction. */
  runTransaction<T>(
...
export class SessionSearchService {
  ...
  constructor(private readonly host: SessionSearchHost) {}

  async initializeSearchState(): Promise<void> {
    await this.host.runTransaction(async (db) => {
    ...
    const timer = setTimeout(() => {
      ...
      const active = (async () => {
        ...
        try {
          ...
          ztoolkit.log(
            "[SessionSearchService] Search backfill slice error:",
            error,
          );
          shouldContinue = true;
