...
- The presentation schema may remain advertised to keep the provider request stable, but execution is locked before confirmation. Do not call presentation in the same model response as request_presentation. If the launcher reports that the user confirmed, call presentation in the following model round with only the authorized sourceItemKey. The app-owned authorization freezes the resolved paper and the settings the user confirmed, and remains valid for the bounded internal retry attempts in that turn.
