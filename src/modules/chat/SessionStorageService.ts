 ...
 *
 * Full-text search + search-index backfill live in the SessionSearchService
 * collaborator (exposed as `.search`); shared row/projection primitives live
 * in session-storage-primitives.
 */
...
import {
  ...
  SEARCH_PROJECTION_WRITE_SENTINEL_VERSION,
  serializeEvidenceRecords,
  serializeSourceItemKeys,
  type MessageStorageRow,
  type QueryableDatabase,
} from "./session-storage-primitives";

// Preserve the module's historical public surface: these were exported from
// SessionStorageService before the primitives split, and external callers
// (e.g. evidence/quoted-message tests) still import them from here.
export {
  mapMessageRowToChatMessage,
  type MessageStorageRow,
} from "./session-storage-primitives";
...
export class SessionStorageService {
  ...
  /**
   * Chat-history search + backfill subsystem. Exposed directly so callers use
   * `sessionStorage.search.searchHistoryGroups(...)`. Injected with a lazy
   * `ensureInitialized` so a search query first guarantees storage init.
   */
  readonly search = new SessionSearchService({
