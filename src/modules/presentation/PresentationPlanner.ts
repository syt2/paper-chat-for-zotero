...
export const PresentationIntentSchema = Type.Object(
  ...
  },
  {
    ...
    description:
      "A lightweight request to create an editable six-page presentation from the current Zotero paper. PaperChat performs detailed planning, figure selection, rendering, and visual review internally.",
  },
);
...
export function buildPresentationPlannerSystemPrompt(): string {
  return [
    ...
    "Create a premium, editable, evidence-first six-page research presentation. The cover is automatic, so return exactly five content slides and never create a second cover.",
    "The deck must tell one cumulative argument: research problem and gap, method, experimental evidence, ablation or limitations, then conclusion and implications.",
    ...
    "Use at least three real PDF figure placements across at least two content slides, at least two distinct crops, and a real cover hero. Allocate every content-slide evidence figure before choosing the cover hero: scientific evidence on a content slide always has priority over cover decoration. Never reuse the same Figure/Table or the same automatic crop inside one gallery, on the cover and a content slide, or across two content slides. Different non-overlapping subfigure crops are allowed only when the crop coordinates are explicit.",
  ...
  if (request.repair) {
    ...
    sections.unshift(
      [
        ...
        "Fix only the listed contract problems while preserving valid paper evidence and the six-page narrative.",
